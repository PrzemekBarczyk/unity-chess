...
namespace Backend
{
	internal static class ZobristKey
	{
		...
		internal static readonly ulong[,,,] colorPiecePositionCombinations = new ulong[PIECE_COLORS, PIECE_TYPES, Board.FILES, Board.RANKS];
		internal static readonly ulong[] castlingRights = new ulong[4];
		internal static readonly ulong[] enPassantFiles = new ulong[8];
		internal static readonly ulong sideToMove;
		...
		internal static ulong CalculateZobristKey(Board board, PieceManager pieceManager, ColorType currentPlayerColor)
		{
			ulong zobristKey = 0;
